...
func (m *MQ) keepalive() {
	...
	case err := <-m.closeC:
		if err == nil {
			log.Errorf(err, "[ERROR] MQ: Disconnected with MQ, but Error detail is nil")
		} else {
			log.Errorf(err, "[ERROR] MQ: Disconnected with MQ, code:%d, reason:%s\n",
				err.Code, err.Reason)
		}
		...
		for i := 0; i < maxRetry; i++ {
			time.Sleep(time.Second)
			if _, e := m.Open(); e != nil {
				log.Errorf(e, "[ERROR] MQ: Connection recover failed for %d times", i+1)
				continue
			}
			log.Infof("[INFO] MQ: Connection recover OK. Total try %d times\n", i+1)
			return
		}
		log.Errorf(errors.New("over maxRetry"),
			"[ERROR] MQ: Try to reconnect to MQ failed over maxRetry(%d), so exit.", maxRetry)
	}
}
