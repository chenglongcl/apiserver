...
func (c *Consumer) consume(opt *ConsumeOption, notifyErr chan<- error) {
	for idx, eb := range c.exchangeBinds {
		...
		for i, b := range eb.Bindings {
			if b == nil {
				notifyErr <- fmt.Errorf("MQ: Binding[%d] is nil, ExchangeBinds[%d], consumer(%s)",
					i, idx, c.name)
				continue
			}
			for qi, q := range b.Queues {
				if q == nil {
					notifyErr <- fmt.Errorf("MQ: Queue[%d] is nil, ExchangeBinds[%d], Biding[%d], consumer(%s)",
						qi, idx, i, c.name)
					continue
				}
				delivery, err := c.ch.Consume(q.Name, "", opt.AutoAck, opt.Exclusive, opt.NoLocal,
					opt.NoWait, opt.Args)
				if err != nil {
					notifyErr <- fmt.Errorf("MQ: Consumer(%s) consume queue(%s) failed, %v",
						c.name, q.Name, err)
					continue
				}
...
func (c *Consumer) keepalive() {
	...
	case err := <-c.closeC:
		if err == nil {
			log.Errorf(err, "[ERROR] MQ: Consumer(%s)'s channel was closed, but Error detail is nil",
				c.name)
		} else {
			log.Errorf(err, "[ERROR] MQ: Consumer(%s)'s channel was closed, code:%d, reason:%s",
		...
		for i := 0; i < maxRetry; i++ {
			...
			if c.mq.State() != StateOpened {
				log.Warnf("[WARN] MQ: Consumer(%s) try to recover channel for %d times, but mq's state != StateOpened",
					c.name, i+1)
				continue
			}
			if e := c.Open(); e != nil {
				log.Warnf("[WARN] MQ: Consumer(%s) recover channel failed for %d times, Err:%v", c.name, i+1, e)
				continue
			}
			log.Infof("[INFO] MQ: Consumer(%s) recover channel OK. Total try %d times", c.name, i+1)
			return
		}
		log.Errorf(errors.New("over maxRetry"),
			"[ERROR] MQ: Consumer(%s) try to recover channel over maxRetry(%d), so exit",
			c.name, maxRetry)
	}
}
