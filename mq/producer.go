...
func (p *Producer) keepalive() {
	...
	case err := <-p.closeC:
		if err == nil {
			log.Errorf(err, "[ERROR] MQ: Producer(%s)'s channel was closed, but Error detail is nil",
				p.name)
		} else {
			log.Errorf(err, "[ERROR] MQ: Producer(%s)'s channel was closed, code:%d, reason:%s\n",
		...
		for i := 0; i < maxRetry; i++ {
			...
			if p.mq.State() != StateOpened {
				log.Warnf("[WARN] MQ: Producer(%s) try to recover channel for %d times, but mq's state != StateOpened\n",
					p.name, i+1)
				continue
			}
			if e := p.Open(); e != nil {
				log.Warnf("[WARN] MQ: Producer(%s) recover channel failed for %d times, Err:%v\n",
					p.name, i+1, e)
				continue
			}
			log.Infof("[INFO] MQ: Producer(%s) recover channel OK. Total try %d times\n", p.name, i+1)
			return
		}
		log.Errorf(errors.New("over maxRetry"),
			"[ERROR] MQ: Producer(%s) try to recover channel over maxRetry(%d), so exit\n", p.name, maxRetry)
	}
}
